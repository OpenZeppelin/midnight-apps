...
describe("Liquidity Calculations", () => {
	...
	describe("calculateAmountOut", () => {
		...
		it("should throw error for invalid amounts", () => {
			expect(() => calculateAmountOut(0n, 1000n, 1000n)).toThrow(
				"Invalid amounts or reserves",
			);
			expect(() => calculateAmountOut(1000n, 0n, 1000n)).toThrow(
				"Invalid amounts or reserves",
			);
			expect(() => calculateAmountOut(1000n, 1000n, 0n)).toThrow(
				"Invalid amounts or reserves",
			);
		});

		it("should throw error for invalid fee", () => {
			expect(() => calculateAmountOut(1000n, 1000n, 1000n, -1)).toThrow(
				"Invalid fee",
			);
			expect(() => calculateAmountOut(1000n, 1000n, 1000n, 10001)).toThrow(
				"Invalid fee",
			);
		});
	});
