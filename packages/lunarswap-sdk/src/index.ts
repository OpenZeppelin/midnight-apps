 ...
 */
export function computeAmountOutMin(expectedOutput: bigint, slippageToleranceBps: number): bigint {
	const SLIPPAGE_DENOMINATOR = 10_000;
	const slippageMultiplier = BigInt(SLIPPAGE_DENOMINATOR - slippageToleranceBps);
 ...
 */
export function computeAmountInMax(expectedInput: bigint, slippageToleranceBps: number): bigint {
	const SLIPPAGE_DENOMINATOR = 10_000;
	const slippageMultiplier = BigInt(SLIPPAGE_DENOMINATOR + slippageToleranceBps);
	return (expectedInput * slippageMultiplier + BigInt(SLIPPAGE_DENOMINATOR - 1)) / BigInt(SLIPPAGE_DENOMINATOR);
}
