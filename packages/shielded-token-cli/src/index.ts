...
export const run = async (
	...
	try {
		let proofServer: StaticProofServerContainer | undefined;
		if (config.networkId !== undefined) {
			const existingPort = process.env.PROOF_SERVER_PORT ?? "6300";
		...
		if (fromEnvRecovery) {
			...
			logger.info(
				"Recovered addresses match TEST_SHIELDED_ADDRESS and TEST_DUST_ADDRESS",
			);
		}
		...
		await mainLoop(providers, config.zkConfigPath, rli, logger, walletProvider);
	} catch (e) {
		logError(logger, e);
		logger.info("Exiting...");
	} finally {
