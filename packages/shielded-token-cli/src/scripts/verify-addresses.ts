...
const networkId = (): string => process.env.TEST_NETWORK_ID ?? "preprod";
...
export function verifyAddresses(overrideNetworkId?: string): void {
	...
	assert(
		recoveryPhrase,
		"TEST_RECOVERY_PHRASE environment variable is required",
	);
	assert(
		expectedShielded,
		"TEST_SHIELDED_ADDRESS environment variable is required",
	);
	...
	const shieldedKeyResult = hdWallet
		...
		.deriveKeyAt(0);
	assert(
		shieldedKeyResult.type === "keyDerived",
		"Failed to derive shielded key",
	);
