import type { U256 } from '@src/artifacts/math/test/mocks/contracts/Vector32.mock/contract/index.js';
import { Vector32Simulator } from '@src/math/test/mocks/Vector32Simulator.js';
import {
  MAX_UINT64,
  MAX_UINT128,
  MAX_UINT256,
...
let vector32Simulator: Vector32Simulator;

const setup = () => {
  vector32Simulator = new Vector32Simulator();
};
...
describe('Vector32', () => {
  ...
  describe('toU256', () => {
    test('should convert zero vector to zero U256', () => {
      ...
      const result = vector32Simulator.toU256(vec);
  ...
  describe('toBytes', () => {
    test('should convert zero vector to zero bytes', () => {
      const vec = toVec32(0n);
      const result = vector32Simulator.toBytes(vec);
    ...
    test('should roundtrip with toU256', () => {
      ...
      const vec = toVec32(value);
      const u256 = vector32Simulator.toU256(vec);
      const bytes = vector32Simulator.toBytes(vec);
