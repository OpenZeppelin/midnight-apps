...
export function useViewPreference() {
  const [viewPreference, setViewPreference] = useState<ViewPreference>('horizontal');
  ...
  useEffect(() => {
    ...
    window.addEventListener('view-preference-changed', handleViewPreferenceChange as EventListener);
    
    return () => {
      window.removeEventListener('view-preference-changed', handleViewPreferenceChange as EventListener);
    };
  }, []);
